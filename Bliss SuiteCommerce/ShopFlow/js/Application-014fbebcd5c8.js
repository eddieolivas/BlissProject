...
define('Categories', function ()
{
    ...
    // Customization by Matias
    // Checks if category has subcategories
  , hasChilds: function (path) 
    {
      var category = (path)? _.last(path.split('/')) : [],
          tree = this.tree;

      if (tree[category] && !_.isUndefined(tree[category].categories) && tree[category].categories.length) 
      {
        return true;
      } else {
        return false;
...
define('Home', function ()
{
	...
	,	title: _('Luxury Home Décor and Furniture | Bliss Home & Design').translate()
